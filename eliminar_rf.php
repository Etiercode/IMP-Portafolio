...
if (!empty($_POST)) {
    ...
    $query_delete = mysqli_query($conexion, "DELETE FROM reportes_f WHERE id_reporte_f='$idReporte'");
    if ($query_delete) {
        echo '
        <script>
        ...
        header("location: reportes_f.php");
    } else {
        echo '
        <script>
...
if (empty($_REQUEST['id'])) {
    ...
    $id_reporte_f = $_REQUEST['id'];

    $sql = mysqli_query($conexion, "SELECT * FROM reportes_f 
    WHERE id_reporte_f=$id_reporte_f");

    $result = mysqli_num_rows($sql);

    if ($result > 0) {
        while ($data = mysqli_fetch_array($sql)) {
            $id_reporte = $data['id_reporte_f'];
            $mensaje = $data['mensaje_f'];
            $id_funcionario_r = $data['id_asignador_f'];
        }
    } else {
        header("location: reportes_f.php");
    }
